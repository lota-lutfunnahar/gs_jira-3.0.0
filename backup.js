...
function Jira() {
  ...
  this.JiraFetch = function(resource)
  {
    ...
    var token = 'bC5uYWhhckBwaXBlbGluZXNlY3VyaXR5Lm5ldDp6WGtkeW5FREs3WDVZSjNJV0FsTkQyQTU=';
    var args = {
  ...
  this.GetSprintDetails = function(url,boardid,sprintname)
  {
    ...
    console.log('sprints name', sprints)
    console.log('sprints name is', sprintname)

    sprintid= 'Gravwell Sprint 1'
    
    sprintname = sprintid.toLowerCase();
    console.log(sprintname.toLowerCase());
    for(var i=0;i<sprints.length;i++)
    {
      //console.log(sprintname+" #  "+sprints[i].name.toLowerCase());
      if(sprints[i].name.toLowerCase() === sprintname)
      {
        console.log(sprints[i].name.toLowerCase());
        return sprints[i];
      } 
    }
    return null;
...
function Main() 
{
  ...
  var statustotrack = ['To Do', 'In Progress', 'In testing', 'Done'];
